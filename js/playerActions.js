define([], function () {
  ...
  var inUpcoming_grace = function (predict) {
    return function (hit) {
      ...
      let result = hit.score < 0 && dx * dx + dy * dy < r * r && Math.abs(predict.time - hit.time) < playback.MehTime;
      ...
      if (playback.autoplay) {
        ...
        while (playback.auto.curid < playback.hits.length && playback.hits[playback.auto.curid].time < time) {
          if (playback.hits[playback.auto.curid].score < 0) {
            ...
            if (playback.hits[playback.auto.curid].type == "spinner") playback.game.mouseY -= spinRadius;
