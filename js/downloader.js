...
function startpreview(box) {
  ...
  for (let i = 0; i < audios.length; ++i) if (audios[i].softstop) audios[i].softstop();
  ...
  let fadeIn = setInterval(function () {
    if (a.volume < volume) a.volume = Math.min(volume, a.volume + 0.05 * volume);
    else clearInterval(fadeIn);
  }, 30);
  let fadeOut = setInterval(function () {
    if (a.currentTime > 29.3) a.volume = Math.max(0, a.volume - 0.05 * volume);
