...
define([
  ...
  function Playback(game, osu, track) {
    ...
    if (this.modhidden) {
      for (let i = 0; i < this.hits.length; i++) {
        if (this.hits[i].approach && i > 0 && this.hits[i - 1].type != "spinner") this.hits[i].approach.visible = false;
      }
    }
    if (this.hideNumbers) {
      for (let i = 0; i < this.hits.length; i++) {
        if (this.hits[i].numbers) {
          for (let j = 0; j < this.hits[i].numbers.length; ++j) this.hits[i].numbers[j].visible = false;
        }
      }
    }
    ...
    this.updateHitCircle = function (hit, time) {
      ...
      if (hit.score > 0 && hit.enableflash) {
        ...
        hit.burst.alpha =
          ...
          clamp01(timeAfter < this.flashFadeInTime ? timeAfter / this.flashFadeInTime : 1 - (timeAfter - this.flashFadeInTime) / this.flashFadeOutTime);
    ...
    this.updateSlider = function (hit, time) {
      ...
      if (-diff >= 0 && -diff <= hit.fadeOutDuration + hit.sliderTimeTotal) {
        ...
        if (hit.repeat > 1) {
          ...
          if (hit.reverse_b) hit.reverse_b.visible = hit.currentRepeat < finalrepfromB;
    ...
    this.render = function (timestamp) {
      ...
      if (typeof time !== "undefined") {
        let nextapproachtime =
          waitinghitid < this.hits.length && this.hits[waitinghitid].time - (this.hits[waitinghitid].approachTime || this.approachTime) > time
            ? this.hits[waitinghitid].time - (this.hits[waitinghitid].approachTime || this.approachTime)
            : -1;
