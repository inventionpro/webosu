...
var NSaddBeatmapList = {
  ...
  addStarRings: function (box, data) {
    ...
    if (stars.length <= 13) {
      for (let i = 0; i < stars.length; i++) {
        ...
        if (s.length > 0) difficultyRing.classList.add(s);
  ...
  addMoreInfo: async function (box, data) {
    ...
    data.beatmaps = data.beatmaps.filter(function (o) {
      return o.mode_int == 0;
    });
    data.beatmaps = data.beatmaps.sort(function (a, b) {
      return Math.sign(a.difficulty_rating - b.difficulty_rating);
    });
    box.data = data;
